...
import type { LifeItem } from "@/domain/life-item";
...
export function LifeCard({ item }: { item: LifeItem }) {
  return <article className="life-card">
    <header><span className={`journey-avatar ${item.type}`}><SubjectIcon type={item.type} /></span><div><small>{item.role === "dependent" ? "Family member" : item.type}</small><h3>{item.displayName}</h3><p>{item.needs.length} {item.needs.length === 1 ? "area" : "areas"} organised</p></div>{item.completed ? <em><CheckCircle2 />All caught up</em> : <em>{item.actions.length} to do</em>}</header>
    ...
    <footer><Link href={`/life/${encodeURIComponent(item.entityId)}`}>See everything for {item.displayName}<ArrowRight /></Link></footer>
  </article>;
}
