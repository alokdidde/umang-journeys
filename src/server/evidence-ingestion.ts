...
export async function createSampleEvidence(type: EvidenceType, facts: Record<string, string>): Promise<EvidenceInput> {
  ...
  page.drawText("UMANG LIFE · EVALUATION ONLY", { x: 62, y: 748, size: 10, font: bold, color: rgb(0.75, 0.18, 0.2) });
  ...
  return {
    ...
    checks: [
      ...
      { label: "Record match", status: "passed", detail: "Generated from details confirmed for this person or thing." },
...
export async function ingestUploadedEvidence(
  ...
  const comparisons = (matchFields[type] ?? []).flatMap(([documentKey, factKey]) => {
    ...
    return [{
      ...
      detail: passed ? "Matches the confirmed value." : `Document says “${documentValue}”; the saved record says “${journeyValue}”.`,
    }];
  });
