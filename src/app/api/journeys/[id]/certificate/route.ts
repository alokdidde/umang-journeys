...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (id !== "demo-new-baby") return NextResponse.json({ code: "JOURNEY_NOT_FOUND" }, { status: 404 });
  ...
  const fields = [
    ["Child's name", "Aarav Sharma"], ["Registration number", "BR-DEMO-2026-7429"],
    ["Date of birth", "24 August 2026"], ["Place of birth", "Hyderabad, Telangana"],
    ["Hospital", "Apollo Hospital (synthetic)"], ["Date of registration", "25 August 2026"],
  ];
  ...
  return new NextResponse(Buffer.from(bytes), {
    headers: { "content-type": "application/pdf", "content-disposition": "attachment; filename=UMANG-DEMO-birth-certificate.pdf", "cache-control": "no-store" },
  });
}
