...
const bodySchema = z.object({ approved: z.boolean(), targetJourneyId: z.string().optional(), targetEntityId: z.string().optional(), fields: z.record(z.string(), z.string().max(300)).optional() });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await documentAssistant.apply(sessionId, id, parsed.data.approved, { targetJourneyId: parsed.data.targetJourneyId, targetEntityId: parsed.data.targetEntityId, fields: parsed.data.fields });
